...
with st.sidebar:
    ...
    k = st.number_input('Input K-factor', min_value=1, max_value=200, step=1, value=24, placeholder='Enter a integer')
    ksi = st.number_input('Input xi', min_value=100, max_value=1000, step=1, value=400, placeholder='Enter a integer')
    s = st.selectbox('Input score type:', options=['delta', 'thirds'], index=0)
    show_surfaces = st.checkbox('Show surface ratings', value=False)
    initial_elo = st.number_input('Initial Elo rating', min_value=0, max_value=5000, step=1, value=1500, placeholder='Enter a integer')
