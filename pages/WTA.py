...
# Load data \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
matches_df = pd.read_csv('web_data/clean_wta_matches.csv')
players_df = pd.read_csv('web_data/clean_wta_players.csv')
initial_ranking = pd.read_csv('web_data/wta_initial_ranking.csv')
initial_elo_history = pd.read_csv('web_data/wta_initial_elo_history.csv')
...
if 'wta_ranking' not in ss:
    ss['wta_ranking'] = initial_ranking

if 'wta_elo_history' not in ss:
    ss['wta_elo_history'] = initial_elo_history
...
if 'wta_selected_player_names' not in ss:
...
if run_simulation:
    ss['wta_ranking'], ss['wta_elo_history'] = ut.simulate_tour(matches_df, players_df, k, ksi, s, initial_elo, min_games)
...
# Histogram
ut.plot_elos_histogram(ss['wta_ranking'], col2)
...
ss['wta_selected_player_names'] = st.multiselect('Selecciona els jugadors que vols veure:', options=player_dict_options.keys(), default=top_five_options.keys(), key='year_list', max_selections=10)
selected_player_ids = [player_dict_options[key] for key in ss['wta_selected_player_names']]
...
chart2 = ut.plot_elo_history_date_st(ss['wta_ranking'], ss['wta_elo_history'], selected_player_ids, ss['wta_start_year'], ss['wta_end_year'])
st.altair_chart(chart2, use_container_width=True)

# Elo history along n-games
chart3 = ut.plot_elo_history_ngames_st(ss['wta_ranking'], ss['wta_elo_history'], selected_player_ids)
